...
for run in range(0,500):
    ...
    time.sleep(2)
    path=r'C:\Users\matth\PhD\Fall 2024\SSIE 501-SSE 440\Programming\ScrapingOutput\FirstTwoTransitions\\'
    blackbox_output = run_black_box(driver, steps=1)
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d"+'_'+"%H-%M-%S")
    blackbox_output.to_csv(path+'blackbox_first_transitions_'+str(run)+'.csv')  
