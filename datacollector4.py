...
for run in range(195,200):
    ...
    time.sleep(5)
    path=r'C:\Users\matth\PhD\Fall 2024\SSIE 501-SSE 440\Programming\ScrapingOutput\DataCollector4\\'
    blackbox_output = run_black_box(driver, steps=2000)
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d"+'_'+"%H-%M-%S")
    blackbox_output.to_csv(path+'blackbox_'+str(run)+'.csv')  
...
input("Runs Complete! Press Enter to Close the Program....")
